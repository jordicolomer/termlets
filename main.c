...
void on_mouse_down(int x, int y) {
    ...
    if (wg != NULL) {
        LOG_INFO(
            "wg id:%s wg->left:%d wg->right:%d wg->top:%d wg->bottom:%d wg->width:%d wg->height:%d",
            wg->id, wg->left, wg->right, wg->top, wg->bottom, wg->width, wg->height);
...
void on_mouse_up() {
    ...
    if (focused_cursor != NULL)
        while (focused_cursor->on_mouse_up == NULL && focused_cursor->focused != NULL)
        ...
        if (c == 27) {
            ...
            if (sscanf(seq, "[<%d;%d;%d%c", &btn, &x, &y, &type) == 4) {
                ...
                if (dragging == 0 && btn == 64 && type == 'M') { // scroll wheel down
                    Window *focused_cursor = focused;
                    if (focused_cursor != NULL)
                        while (focused_cursor->scroll_wheel_up == NULL &&
                               focused_cursor->focused != NULL)
                            focused_cursor = focused_cursor->focused;
                ...
                if (dragging == 0 && btn == 65 && type == 'M') { // scroll wheel down
                    Window *focused_cursor = focused;
                    if (focused_cursor != NULL)
                        while (focused_cursor->scroll_wheel_up == NULL &&
                               focused_cursor->focused != NULL)
