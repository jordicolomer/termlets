...
Window *Menu_list_windows(Window *self, Tabs *tabs, struct Window *menuItem)
{
    Window *menu = malloc(sizeof *menu);
    Window_init(menu, -1, -1, -1, -1, -1, -1);
    menu->left = menuItem->calculated.x - self->calculated.x;
    ...
    while (tab != NULL)
    {
        ...
        //win->data2 = menu;
        //win->on_mouse_down = Menu_menu_windows_item;
        win->lambda = create_lambda(Menu_list_windows_item_selected, 2, tab, menu);
...
void Menu_add_windows2(Menu *self, char *name, Tabs *tabs)
{
    // Window * Menu_add_element(Menu * self, char * name, Menu_lambda);
    int len = strlen(name);
    self->offset += 1;
    Window *window_menu_item = Window_add_widget(self, self->offset, -1, 0, -1, len, 1, name, 232, 253);
    self->offset += len + 1;

    //window->data = Menu_menu_windows;
    //window->data2 = self->parent;
    //window->data3 = tabs;
    window_menu_item->lambda = create_lambda(Menu_list_windows, 3, self, tabs, window_menu_item);
