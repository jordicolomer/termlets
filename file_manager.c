...
void item_clicked(Window *wg, int x, int y) {
    ...
    LOG_INFO("item_clicked: self=%p file_item=%p is_dir=%d path=%s", self, file_item,
             file_item->is_dir, file_item->path);
...
void FileExplorer_list_files(ExplorerWindow *self, char *dire) {
    ...
    fm->virtual_height = j;
    LOG_INFO("FileExplorer_list_files: added %d files, virtual_height=%d, calculated.height=%d, "
             "fm->head=%p",
...
void FileExplorer_copy_directory(ExplorerWindow *self) { clipboard_copy(self->path); }
...
Window *ExplorerWindow_rename(ExplorerWindow *self) {
    ...
    line_edit->win.lambda =
        create_lambda(ExplorerWindow_rename_complete, 3, self, line_edit, selected->path);
...
Window *FileExplorer_menu_paste(ExplorerFrame *self) {
    if (paste_source == NULL)
        return NULL;
    if (paste_source->fm == NULL)
        return NULL;
    ExplorerWindow *ew = self->tabs->focused;
    ...
    while (current != NULL) {
        if (current->is_selected == 1) {
            ...
            if (paste_operation == 1)
                rename(current->path, dst);
            if (paste_operation == 2)
                copy_file(current->path, dst);
            free(dst);
        }
        current = (FileItemWindow *)current->win.next;
    }
...
Window *FileExplorer_menu_new(ExplorerFrame *self) { LOG_INFO("FileExplorer_menu_new %p", self); }
...
void FileExplorer_close(ExplorerFrame *self) {
    Tabs *tabs = self->tabs;
...
Window *FileExplorer_menu(ExplorerFrame *self) {
    ...
    Menu_add_element(file, " 📄 New File",
                     create_lambda(ExplorerFrame_on_selected, 2, self, FileExplorer_new_file));
    Menu_add_element(file, " 📁 New Folder",
                     create_lambda(ExplorerFrame_on_selected, 2, self, FileExplorer_new_folder));
    ...
    Menu_add_element(file, " ❌ Close Tab", create_lambda(FileExplorer_close, 1, self));
    ...
    Menu_add_element(edit, " 🔪 Cut            Ctrl+X",
                     create_lambda(FileExplorer_menu_cut, 1, self));
    Menu_add_element(edit, " 📋 Copy           Ctrl+C",
                     create_lambda(FileExplorer_menu_copy, 1, self));
    Menu_add_element(edit, " 📌 Paste          Ctrl+V",
                     create_lambda(FileExplorer_menu_paste, 1, self));
    Menu_add_element(edit, " ❌ Delete         Backspace",
                     create_lambda(FileExplorer_menu_delete, 1, self));
    Menu_add_element(edit, " 📝 Rename         Ctrl+R",
                     create_lambda(FileExplorer_menu_rename, 1, self));
    Menu_add_element(edit, "", NULL);
    Menu_add_element(edit, " 📋 Copy Name      Ctrl+C",
                     create_lambda(ExplorerFrame_on_selected, 2, self, FileExplorer_copy_name));
    Menu_add_element(
        edit, " 📋 Copy Directory",
        create_lambda(ExplorerFrame_on_selected, 2, self, FileExplorer_copy_directory));
    Menu_add_element(edit, " 📋 Copy Path",
                     create_lambda(ExplorerFrame_on_selected, 2, self, FileExplorer_copy_path));
    Menu_add_element(edit, "", NULL);
    ...
    Menu_add_element(view, " ↓ Sort By Date Modified",
                     create_lambda(FileExplorer_sort_by, 2, self, SORT_BY_DATE));
    Menu_add_element(view, " ↓ Sort By Size",
                     create_lambda(FileExplorer_sort_by, 2, self, SORT_BY_SIZE));
...
Window *FileExplorer_toolbar(ExplorerFrame *self) {
    ...
    Menu_add_element(toolbar_menu, " 📝 Edit ",
                     create_lambda(ExplorerFrame_on_selected, 2, self, FileExplorer_edit));
    Menu_add_element(toolbar_menu, " 💻 Terminal ",
                     create_lambda(ExplorerFrame_on_selected, 2, self, FileExplorer_terminal));
