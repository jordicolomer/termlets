...
void task_on_mouse_down(struct Window *w, int x, int y) {
    if (selectedTask != NULL)
        selectedTask->bg = unselectedTaskColor;
...
void cycle_task() {
    ...
    if (selectedTask->next == NULL)
        task = selectedTask->parent->head->next;
    TaskBar_switch(task);
...
void vterminal_new(char *cwd) {
    Window *fm =
        VTermTerminal_new(window_x, -1, window_y, -1, root->width / 2, 2 * root->height / 3, cwd);
    update_coords(root->width / 2, 2 * root->height / 3);
    // window_x += 10;
    // window_y += 3;
    fm->parent = root;
...
Window *TaskBar_new() {
    ...
    Window *file_manager =
        Window_add_widget(startMenu, 0, -1, idx++, -1, 18, 1, "📁 File Manager", 0, taskbar_color);
