...
void window_manager_new(){
  ...
  Window *task = TaskBar_new_task("🪟 Window Manager", fm);
  fm->data = task;
}

void window_manager_mouse_down(struct Window *w, int x, int y){
  Window *startMenu = w->data;
  startMenu->hidden = 1;
  open_menu = NULL;
...
Window * TaskBar_new(){
  ...
  Window *chess = Window_add_widget(startMenu, 0, -1, idx++, -1, 18, 1, "🏁 Chess", 0, taskbar_color);
  ...
  Window *file_manager = Window_add_widget(startMenu, 0, -1, idx++, -1, 18, 1, "📁 File Manager", 0, taskbar_color);
  file_manager->on_mouse_down = file_manager_mouse_down;
  file_manager->data = startMenu;

  /*Window *terminal = Window_add_widget(startMenu, 0, -1, 1, -1, 18, 1, "💻 Terminal", 0, taskbar_color);
  terminal->on_mouse_down = terminal_mouse_down;
  terminal->data = startMenu;*/

  Window *vterminal = Window_add_widget(startMenu, 0, -1, idx++, -1, 18, 1, "💻 Terminal", 0, taskbar_color);
  vterminal->on_mouse_down = vterminal_mouse_down;
  vterminal->data = startMenu;

  Window *quit = Window_add_widget(startMenu, 0, -1, idx++, -1, 18, 1, "⏻  Quit", 0, taskbar_color);
